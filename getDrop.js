...
import{listenForDropMintedEvents} from "./minted.js"
...
async function listenForDropCreatedEvents() {
    try {
        const filter = contract.filters.DropCreated();

        contract.on(filter, async (event) => {
            const eventId = event.args.id.toString();
            const drop = event.args.drop;

            const dropId = drop.dropId.toString();
            const price = ethers.formatEther(drop.price.toString());

            console.log(`Event ID: ${eventId}`);
            console.log('Drop Details:', drop);
            console.log(`Drop ID: ${dropId}`);
            console.log(`Price: ${price} TIA`);
            console.log("\n");
            await call_multiple(dropId,price.toString())
            await listenForDropMintedEvents(dropId,price.toString()).catch(error => {
                    console.error("Error starting event listener:", error);
                }); 
        });

        console.log("Listening for DropCreated events...");

    } catch (error) {
        console.error("Error setting up event listener:", error);
        setTimeout(listenForDropCreatedEvents, 60000);
    }
}
listenForDropCreatedEvents().catch(error => {
    console.error("Error starting event listener:", error);
});
